...
description: |
  ...
  Use when reviewing Snowflake code for issues, onboarding new developers,
  or auditing existing Snowflake integrations for best practices violations.
...
app.post('/webhook',
  ...
  (req, res) => {
    const signature = req.headers['x-snowflake-signature'];
    if (!verifySnowflakeSignature(req.body, signature)) {
...
const result = await snowflakeClient.get(id);
...
try {
  ...
  if (error instanceof SnowflakeNotFoundError) {
    return null;
  }
  if (error instanceof SnowflakeRateLimitError) {
...
const client = new SnowflakeClient({
  ...
  baseUrl: 'https://api.snowflake.com',  // Can't change for staging
});
```
...
const client = new SnowflakeClient({
  ...
  baseUrl: process.env.SNOWFLAKE_BASE_URL || 'https://api.snowflake.com',
});
```
...
### ❌ Anti-Pattern
```typescript
// When Snowflake is down, every request hangs
for (const user of users) {
  await snowflakeClient.sync(user);  // All timeout sequentially
}
```
...
const breaker = new CircuitBreaker(snowflakeClient.sync, {
  timeout: 10000,
...
### ❌ Anti-Pattern
```typescript
// Entire feature broken if Snowflake is down
const recommendations = await snowflakeClient.getRecommendations(userId);
...
let recommendations;
try {
  recommendations = await snowflakeClient.getRecommendations(userId);
} catch (error) {
  recommendations = await getFallbackRecommendations(userId);
  reportDegradedService('snowflake', error);
}
...
## Resources
- [Snowflake Security Guide](https://docs.snowflake.com/security)
