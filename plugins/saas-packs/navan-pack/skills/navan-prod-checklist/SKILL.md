...
description: |
  ...
  Use when deploying Navan integrations to production, preparing for launch,
  or implementing go-live procedures.
  Trigger with phrases like "navan production", "deploy navan",
  "navan go-live", "navan launch checklist".
...
async function healthCheck(): Promise<{ status: string; navan: any }> {
  ...
  try {
    ...
    return { status: 'healthy', navan: { connected: true, latencyMs: Date.now() - start } };
  } catch (error) {
    return { status: 'degraded', navan: { connected: false, latencyMs: Date.now() - start } };
  }
}
```
...
### Immediate Rollback
```bash
kubectl rollout undo deployment/navan-integration
kubectl rollout status deployment/navan-integration
```
...
## Next Steps
For version upgrades, see `navan-upgrade-migration`.