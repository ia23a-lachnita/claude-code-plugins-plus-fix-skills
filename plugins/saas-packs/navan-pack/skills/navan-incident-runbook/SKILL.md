...
description: |
  ...
  Use when responding to Navan-related outages, investigating errors,
  or running post-incident reviews for Navan integration failures.
  Trigger with phrases like "navan incident", "navan outage",
  "navan down", "navan on-call", "navan emergency", "navan broken".
...
|-------|------------|---------------|----------|
| P1 | Complete outage | < 15 min | Navan API unreachable |
...
```
Navan Integration Issue

We're experiencing issues with our Navan integration.
...
./scripts/navan-debug-bundle.sh

# Export relevant logs
kubectl logs -l app=navan-integration --since=1h > incident-logs.txt

# Capture metrics
curl "localhost:9090/api/v1/query_range?query=navan_errors_total&start=2h" > metrics.json
```

### Postmortem Template
```markdown
## Incident: Navan [Error Type]
**Date:** YYYY-MM-DD
...
Determine if the issue is Navan-side or internal.
...
```bash
curl -sf https://api.yourapp.com/health | jq '.services.navan.status' || echo "UNHEALTHY"
```

## Resources
- [Navan Status Page](https://status.navan.com)
- [Navan Support](https://support.navan.com)

## Next Steps
For data handling, see `navan-data-handling`.