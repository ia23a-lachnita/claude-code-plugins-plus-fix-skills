...
description: |
  ...
  or optimizing request throughput for Salesforce integrations.
  Trigger with phrases like "salesforce performance", "optimize salesforce",
  "salesforce latency", "salesforce caching", "salesforce slow", "salesforce batch".
...
// Usage
for await (const item of paginatedSalesforceList(cursor =>
  salesforceClient.list({ cursor, limit: 100 })
)) {
  await process(item);
}
```
...
```typescript
async function measuredSalesforceCall<T>(
  operation: string,
  fn: () => Promise<T>
): Promise<T> {
...
Measure current latency for critical Salesforce operations.
...
const withPerformance = <T>(name: string, fn: () => Promise<T>) =>
  measuredSalesforceCall(name, () =>
    cachedSalesforceRequest(`cache:${name}`, fn)
  );
```

## Resources
- [Salesforce Performance Guide](https://docs.salesforce.com/performance)
