...
description: |
  Deploy StackBlitz integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying StackBlitz-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy stackblitz", "stackblitz Vercel",
  "stackblitz production deploy", "stackblitz Cloud Run", "stackblitz Fly.io".
...
## Overview
Deploy StackBlitz-powered applications to popular platforms with proper secrets management.
...
### Environment Setup
```bash
# Add StackBlitz secrets to Vercel
...
### Secrets
```bash
# Set StackBlitz secrets
fly secrets set STACKBLITZ_API_KEY=sk_live_***
fly secrets set STACKBLITZ_WEBHOOK_SECRET=whsec_***
...
SERVICE_NAME="stackblitz-service"
REGION="us-central1"
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=STACKBLITZ_API_KEY=stackblitz-api-key:latest
```
...
export function getStackBlitzConfig(): StackBlitzConfig {
  const env = process.env.NODE_ENV || 'development';

  return {
    apiKey: process.env.STACKBLITZ_API_KEY!,
    environment: env as StackBlitzConfig['environment'],
    webhookSecret: process.env.STACKBLITZ_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add stackblitz_api_key "$STACKBLITZ_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set STACKBLITZ_API_KEY="$STACKBLITZ_API_KEY"
    fly deploy
    ;;
esac
```
