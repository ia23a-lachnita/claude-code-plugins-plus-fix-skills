...
// Usage
for await (const item of paginatedGrammarlyList(cursor =>
  grammarlyClient.list({ cursor, limit: 100 })
)) {
  await process(item);
}
```
...
```typescript
async function measuredGrammarlyCall<T>(
  operation: string,
  fn: () => Promise<T>
): Promise<T> {
...
Measure current latency for critical Grammarly operations.
...
const withPerformance = <T>(name: string, fn: () => Promise<T>) =>
  measuredGrammarlyCall(name, () =>
    cachedGrammarlyRequest(`cache:${name}`, fn)
  );
```

## Resources
- [Grammarly Performance Guide](https://docs.grammarly.com/performance)
