...
description: |
  ...
  or implementing environment-specific Intercom configurations.
  Trigger with phrases like "intercom environments", "intercom staging",
  "intercom dev prod", "intercom environment setup", "intercom config by env".
...
INTERCOM_API_KEY=sk_test_dev_***
```

### CI/CD (GitHub Actions)
```yaml
env:
  INTERCOM_API_KEY: ${{ secrets.INTERCOM_API_KEY_${{ matrix.environment }} }}
```

### Production (Vault/Secrets Manager)
```bash
# AWS Secrets Manager
aws secretsmanager get-secret-value --secret-id intercom/production/api-key

# GCP Secret Manager
gcloud secrets versions access latest --secret=intercom-api-key

# HashiCorp Vault
vault kv get -field=api_key secret/intercom/production
```
...
function guardProductionOperation(operation: string): void {
  ...
  if (config.environment !== 'production') {
    console.warn(`[intercom] ${operation} blocked in ${config.environment}`);
...
```typescript
const env = getIntercomConfig();
console.log(`Running in ${env.environment} with ${env.baseUrl}`);
```

## Resources
- [Intercom Environments Guide](https://docs.intercom.com/environments)
