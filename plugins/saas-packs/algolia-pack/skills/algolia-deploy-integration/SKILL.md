...
description: |
  Deploy Algolia integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying Algolia-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy algolia", "algolia Vercel",
  "algolia production deploy", "algolia Cloud Run", "algolia Fly.io".
...
case "$1" in
  vercel)
    vercel secrets add algolia_api_key "$ALGOLIA_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set ALGOLIA_API_KEY="$ALGOLIA_API_KEY"
    fly deploy
    ;;
esac
```
