...
### Environment Setup
```bash
# Add BambooHR secrets to Vercel
...
### Secrets
```bash
# Set BambooHR secrets
fly secrets set BAMBOOHR_API_KEY=sk_live_***
fly secrets set BAMBOOHR_WEBHOOK_SECRET=whsec_***
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=BAMBOOHR_API_KEY=bamboohr-api-key:latest
```
...
export function getBambooHRConfig(): BambooHRConfig {
  ...
  return {
    ...
    environment: env as BambooHRConfig['environment'],
    webhookSecret: process.env.BAMBOOHR_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add bamboohr_api_key "$BAMBOOHR_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set BAMBOOHR_API_KEY="$BAMBOOHR_API_KEY"
    fly deploy
    ;;
esac
```
