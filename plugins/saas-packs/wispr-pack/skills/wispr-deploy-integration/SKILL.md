...
description: |
  Deploy Wispr integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying Wispr-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy wispr", "wispr Vercel",
  "wispr production deploy", "wispr Cloud Run", "wispr Fly.io".
...
## Overview
Deploy Wispr-powered applications to popular platforms with proper secrets management.
...
### Environment Setup
```bash
# Add Wispr secrets to Vercel
vercel secrets add wispr_api_key sk_live_***
vercel secrets add wispr_webhook_secret whsec_***
...
```bash
# Set Wispr secrets
fly secrets set WISPR_API_KEY=sk_live_***
fly secrets set WISPR_WEBHOOK_SECRET=whsec_***
...
export function getWisprConfig(): WisprConfig {
  const env = process.env.NODE_ENV || 'development';

  return {
    apiKey: process.env.WISPR_API_KEY!,
    environment: env as WisprConfig['environment'],
    webhookSecret: process.env.WISPR_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    ...
    ;;
  fly)
    fly secrets set WISPR_API_KEY="$WISPR_API_KEY"
    fly deploy
    ;;
esac
```
