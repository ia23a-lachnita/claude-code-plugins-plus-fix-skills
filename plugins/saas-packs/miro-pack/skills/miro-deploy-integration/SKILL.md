...
description: |
  Deploy Miro integrations to Vercel, Fly.io, and Cloud Run platforms.
...
## Overview
Deploy Miro-powered applications to popular platforms with proper secrets management.
...
fly secrets set MIRO_WEBHOOK_SECRET=whsec_***
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=MIRO_API_KEY=miro-api-key:latest
```
...
export function getMiroConfig(): MiroConfig {
  const env = process.env.NODE_ENV || 'development';

  return {
    apiKey: process.env.MIRO_API_KEY!,
    environment: env as MiroConfig['environment'],
    webhookSecret: process.env.MIRO_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add miro_api_key "$MIRO_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set MIRO_API_KEY="$MIRO_API_KEY"
    fly deploy
    ;;
esac
```
