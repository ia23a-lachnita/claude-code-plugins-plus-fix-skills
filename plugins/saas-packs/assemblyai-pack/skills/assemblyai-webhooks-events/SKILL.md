...
description: |
  Implement AssemblyAI webhook signature validation and event handling.
  Use when setting up webhook endpoints, implementing signature verification,
  or handling AssemblyAI event notifications securely.
  Trigger with phrases like "assemblyai webhook", "assemblyai events",
  "assemblyai webhook signature", "handle assemblyai events", "assemblyai notifications".
...
## Overview
Securely handle AssemblyAI webhooks with signature validation and replay protection.

## Prerequisites
- AssemblyAI webhook secret configured
...
async function isEventProcessed(eventId: string): Promise<boolean> {
  const key = `assemblyai:event:${eventId}`;
...
```bash
# Use AssemblyAI CLI to send test events
assemblyai webhooks trigger resource.created --url http://localhost:3000/webhooks/assemblyai
...
Configure your webhook URL in the AssemblyAI dashboard.
...
# Send test webhook
curl -X POST https://your-ngrok-url/webhooks/assemblyai \
  -H "Content-Type: application/json" \
  -d '{"type": "test", "data": {}}'
```

## Resources
- [AssemblyAI Webhooks Guide](https://docs.assemblyai.com/webhooks)
- [Webhook Security Best Practices](https://docs.assemblyai.com/webhooks/security)
