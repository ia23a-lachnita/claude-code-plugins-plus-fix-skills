...
## Overview
Complete checklist for deploying AssemblyAI integrations to production.
...
- [ ] Error handling covers all AssemblyAI error types
- [ ] Rate limiting/backoff implemented
- [ ] Logging is production-appropriate

### Step 3: Infrastructure Setup
- [ ] Health check endpoint includes AssemblyAI connectivity
...
## Output
- Deployed AssemblyAI integration
...
```typescript
async function healthCheck(): Promise<{ status: string; assemblyai: any }> {
  const start = Date.now();
  try {
    await assemblyaiClient.ping();
    return { status: 'healthy', assemblyai: { connected: true, latencyMs: Date.now() - start } };
  } catch (error) {
    return { status: 'degraded', assemblyai: { connected: false, latencyMs: Date.now() - start } };
  }
}
```
...
### Immediate Rollback
```bash
kubectl rollout undo deployment/assemblyai-integration
kubectl rollout status deployment/assemblyai-integration
```

## Resources
- [AssemblyAI Status](https://status.assemblyai.com)
- [AssemblyAI Support](https://docs.assemblyai.com/support)
