...
description: |
  Configure Webflow local development with hot reload and testing.
  Use when setting up a development environment, configuring test workflows,
  or establishing a fast iteration cycle with Webflow.
...
## Overview
Set up a fast, reproducible local development workflow for Webflow.
...
├── src/
│   ├── webflow/
│   │   ├── client.ts       # Webflow client wrapper
...
import { WebflowClient } from '../src/webflow/client';

describe('Webflow Client', () => {
  it('should initialize with API key', () => {
    const client = new WebflowClient({ apiKey: 'test-key' });
...
- Fast iteration cycle for Webflow development
