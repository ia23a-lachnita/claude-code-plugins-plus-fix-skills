...
description: |
  Deploy Lucidchart integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying Lucidchart-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy lucidchart", "lucidchart Vercel",
  "lucidchart production deploy", "lucidchart Cloud Run", "lucidchart Fly.io".
...
## Overview
Deploy Lucidchart-powered applications to popular platforms with proper secrets management.
...
### Environment Setup
```bash
# Add Lucidchart secrets to Vercel
...
### Secrets
```bash
# Set Lucidchart secrets
fly secrets set LUCIDCHART_API_KEY=sk_live_***
fly secrets set LUCIDCHART_WEBHOOK_SECRET=whsec_***
...
SERVICE_NAME="lucidchart-service"
REGION="us-central1"
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=LUCIDCHART_API_KEY=lucidchart-api-key:latest
```
...
interface LucidchartConfig {
  apiKey: string;
  environment: 'development' | 'staging' | 'production';
  webhookSecret?: string;
}

export function getLucidchartConfig(): LucidchartConfig {
  const env = process.env.NODE_ENV || 'development';

  return {
    apiKey: process.env.LUCIDCHART_API_KEY!,
    environment: env as LucidchartConfig['environment'],
    webhookSecret: process.env.LUCIDCHART_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add lucidchart_api_key "$LUCIDCHART_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set LUCIDCHART_API_KEY="$LUCIDCHART_API_KEY"
    fly deploy
    ;;
esac
```
