...
description: |
  Identify and avoid Notion anti-patterns and common integration mistakes.
  Use when reviewing Notion code for issues, onboarding new developers,
  or auditing existing Notion integrations for best practices violations.
...
## Overview
Common mistakes and anti-patterns when integrating with Notion.

## Prerequisites
- Access to Notion codebase for review
...
### ❌ Anti-Pattern
```typescript
// User waits for Notion API call
app.post('/checkout', async (req, res) => {
  const payment = await notionClient.processPayment(req.body);  // 2-5s latency
  const notification = await notionClient.sendEmail(payment);   // Another 1-2s
...
async function processCheckout(data) {
  const payment = await notionClient.processPayment(data);
  await notionClient.sendEmail(payment);
}
```
...
for (const item of items) {
  await notionClient.process(item);  // Will hit rate limit
}
```
...
for (const item of items) {
  ...
  await limit(() => notionClient.process(item));
}
```
...
try {
  await notionClient.charge(order);
} catch (error) {
  if (error.code === 'NETWORK_ERROR') {
    await notionClient.charge(order);  // Charged twice!
  }
}
```
...
app.post('/webhook',
  ...
  (req, res) => {
    ...
    if (!verifyNotionSignature(req.body, signature)) {
...
const result = await notionClient.get(id);
...
const client = new NotionClient({
  ...
  baseUrl: 'https://api.notion.com',  // Can't change for staging
});
```
...
### ❌ Anti-Pattern
```typescript
// When Notion is down, every request hangs
for (const user of users) {
  await notionClient.sync(user);  // All timeout sequentially
}
```
...
const breaker = new CircuitBreaker(notionClient.sync, {
  timeout: 10000,
...
### ❌ Anti-Pattern
```typescript
// Entire feature broken if Notion is down
const recommendations = await notionClient.getRecommendations(userId);
...
let recommendations;
try {
  recommendations = await notionClient.getRecommendations(userId);
} catch (error) {
  recommendations = await getFallbackRecommendations(userId);
  reportDegradedService('notion', error);
}
