...
description: |
  Apply Notion advanced debugging techniques for hard-to-diagnose issues.
  Use when standard troubleshooting fails, investigating complex race conditions,
  or preparing evidence bundles for Notion support escalation.
...
## Overview
Deep debugging techniques for complex Notion issues that resist standard troubleshooting.
...
```bash
#!/bin/bash
# advanced-notion-debug.sh

BUNDLE="notion-advanced-debug-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BUNDLE"/{logs,metrics,network,config,traces}

# 1. Extended logs (1 hour window)
kubectl logs -l app=notion-integration --since=1h > "$BUNDLE/logs/pods.log"
journalctl -u notion-service --since "1 hour ago" > "$BUNDLE/logs/system.log"

# 2. Metrics dump
curl -s localhost:9090/api/v1/query?query=notion_requests_total > "$BUNDLE/metrics/requests.json"
curl -s localhost:9090/api/v1/query?query=notion_errors_total > "$BUNDLE/metrics/errors.json"

# 3. Network capture (30 seconds)
timeout 30 tcpdump -i any port 443 -w "$BUNDLE/network/capture.pcap" &

# 4. Distributed traces
curl -s localhost:16686/api/traces?service=notion > "$BUNDLE/traces/jaeger.json"

# 5. Configuration state
kubectl get cm notion-config -o yaml > "$BUNDLE/config/configmap.yaml"
kubectl get secret notion-secrets -o yaml > "$BUNDLE/config/secrets-redacted.yaml"
...
async function diagnoseNotionIssue(): Promise<DiagnosisReport> {
  ...
  results.push(await testDNSResolution('api.notion.com'));

  // Layer 3: TLS handshake
  results.push(await testTLSHandshake('api.notion.com'));
...
```typescript
// Detect memory leaks in Notion client usage
...
setInterval(() => {
  ...
  if (heapUsed.length > 60) { // 1 hour at 1/min
    ...
    if (trend > 100 * 1024 * 1024) { // 100MB growth
      console.warn('Potential memory leak in notion integration');
    }
  }
}, 60000);
```
...
class NotionConcurrencyChecker {
...
### Evidence Attached
- [ ] Debug bundle (notion-advanced-debug-*.tar.gz)
...
curl -v https://api.notion.com/health 2>&1 | grep -E "(Connected|TLS|HTTP)"
```

## Resources
- [Notion Support Portal](https://support.notion.com)
