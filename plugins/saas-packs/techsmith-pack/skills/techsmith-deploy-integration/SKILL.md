...
description: |
  Deploy TechSmith integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying TechSmith-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy techsmith", "techsmith Vercel",
  "techsmith production deploy", "techsmith Cloud Run", "techsmith Fly.io".
...
## Overview
Deploy TechSmith-powered applications to popular platforms with proper secrets management.
...
### Environment Setup
```bash
# Add TechSmith secrets to Vercel
...
### Secrets
```bash
# Set TechSmith secrets
fly secrets set TECHSMITH_API_KEY=sk_live_***
fly secrets set TECHSMITH_WEBHOOK_SECRET=whsec_***
...
SERVICE_NAME="techsmith-service"
REGION="us-central1"
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=TECHSMITH_API_KEY=techsmith-api-key:latest
```
...
```typescript
// config/techsmith.ts
interface TechSmithConfig {
  apiKey: string;
  environment: 'development' | 'staging' | 'production';
  webhookSecret?: string;
}

export function getTechSmithConfig(): TechSmithConfig {
  const env = process.env.NODE_ENV || 'development';

  return {
    apiKey: process.env.TECHSMITH_API_KEY!,
    environment: env as TechSmithConfig['environment'],
    webhookSecret: process.env.TECHSMITH_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add techsmith_api_key "$TECHSMITH_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set TECHSMITH_API_KEY="$TECHSMITH_API_KEY"
    fly deploy
    ;;
esac
```
