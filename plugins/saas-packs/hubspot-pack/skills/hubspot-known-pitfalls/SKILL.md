...
description: |
  Identify and avoid HubSpot anti-patterns and common integration mistakes.
  Use when reviewing HubSpot code for issues, onboarding new developers,
  or auditing existing HubSpot integrations for best practices violations.
  Trigger with phrases like "hubspot mistakes", "hubspot anti-patterns",
  "hubspot pitfalls", "hubspot what not to do", "hubspot code review".
...
## Overview
Common mistakes and anti-patterns when integrating with HubSpot.

## Prerequisites
- Access to HubSpot codebase for review
...
### ❌ Anti-Pattern
```typescript
// User waits for HubSpot API call
app.post('/checkout', async (req, res) => {
  const payment = await hubspotClient.processPayment(req.body);  // 2-5s latency
  const notification = await hubspotClient.sendEmail(payment);   // Another 1-2s
...
async function processCheckout(data) {
  const payment = await hubspotClient.processPayment(data);
  await hubspotClient.sendEmail(payment);
}
```
...
for (const item of items) {
  await hubspotClient.process(item);  // Will hit rate limit
}
```
...
for (const item of items) {
  ...
  await limit(() => hubspotClient.process(item));
}
```
...
try {
  await hubspotClient.charge(order);
} catch (error) {
  if (error.code === 'NETWORK_ERROR') {
    await hubspotClient.charge(order);  // Charged twice!
  }
}
```
...
const result = await hubspotClient.get(id);
...
try {
  ...
  if (error instanceof HubSpotNotFoundError) {
    return null;
  }
...
const client = new HubSpotClient({
  ...
  baseUrl: 'https://api.hubspot.com',  // Can't change for staging
});
```
...
```typescript
const client = new HubSpotClient({
  timeout: parseInt(process.env.HUBSPOT_TIMEOUT || '30000'),
  baseUrl: process.env.HUBSPOT_BASE_URL || 'https://api.hubspot.com',
});
```
...
### ❌ Anti-Pattern
```typescript
// When HubSpot is down, every request hangs
for (const user of users) {
  await hubspotClient.sync(user);  // All timeout sequentially
}
```
...
const breaker = new CircuitBreaker(hubspotClient.sync, {
  timeout: 10000,
...
### ❌ Anti-Pattern
```typescript
// Entire feature broken if HubSpot is down
const recommendations = await hubspotClient.getRecommendations(userId);
...
let recommendations;
try {
  recommendations = await hubspotClient.getRecommendations(userId);
} catch (error) {
  recommendations = await getFallbackRecommendations(userId);
  reportDegradedService('hubspot', error);
}
...
## Resources
- [HubSpot Security Guide](https://docs.hubspot.com/security)
