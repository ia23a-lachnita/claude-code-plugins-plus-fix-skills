...
description: |
  Configure HubSpot local development with hot reload and testing.
  Use when setting up a development environment, configuring test workflows,
  or establishing a fast iteration cycle with HubSpot.
  Trigger with phrases like "hubspot dev setup", "hubspot local development",
  "hubspot dev environment", "develop with hubspot".
...
## Overview
Set up a fast, reproducible local development workflow for HubSpot.
...
├── src/
│   ├── hubspot/
│   │   ├── client.ts       # HubSpot client wrapper
...
describe('HubSpot Client', () => {
  it('should initialize with API key', () => {
    const client = new HubSpotClient({ apiKey: 'test-key' });
...
- Fast iteration cycle for HubSpot development
