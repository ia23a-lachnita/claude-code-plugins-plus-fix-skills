...
description: |
  ...
  or optimizing request throughput for CoreWeave integrations.
  Trigger with phrases like "coreweave performance", "optimize coreweave",
  "coreweave latency", "coreweave caching", "coreweave slow", "coreweave batch".
...
// Usage
for await (const item of paginatedCoreWeaveList(cursor =>
  coreweaveClient.list({ cursor, limit: 100 })
)) {
  await process(item);
}
```
...
```typescript
async function measuredCoreWeaveCall<T>(
  operation: string,
  fn: () => Promise<T>
): Promise<T> {
...
Measure current latency for critical CoreWeave operations.
...
const withPerformance = <T>(name: string, fn: () => Promise<T>) =>
  measuredCoreWeaveCall(name, () =>
    cachedCoreWeaveRequest(`cache:${name}`, fn)
  );
```

## Resources
- [CoreWeave Performance Guide](https://docs.coreweave.com/performance)
