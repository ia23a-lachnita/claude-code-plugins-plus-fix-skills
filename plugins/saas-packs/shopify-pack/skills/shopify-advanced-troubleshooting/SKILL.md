...
description: |
  ...
  or preparing evidence bundles for Shopify support escalation.
  Trigger with phrases like "shopify hard bug", "shopify mystery error",
  "shopify impossible to debug", "difficult shopify issue", "shopify deep debug".
...
```typescript
// Detect memory leaks in Shopify client usage
...
setInterval(() => {
  ...
  if (heapUsed.length > 60) { // 1 hour at 1/min
    ...
    if (trend > 100 * 1024 * 1024) { // 100MB growth
      console.warn('Potential memory leak in shopify integration');
    }
  }
}, 60000);
```
...
class ShopifyConcurrencyChecker {
...
### Evidence Attached
- [ ] Debug bundle (shopify-advanced-debug-*.tar.gz)
...
curl -v https://api.shopify.com/health 2>&1 | grep -E "(Connected|TLS|HTTP)"
```

## Resources
- [Shopify Support Portal](https://support.shopify.com)
