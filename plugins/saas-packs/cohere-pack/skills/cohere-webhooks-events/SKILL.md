...
description: |
  ...
  or handling Cohere event notifications securely.
  Trigger with phrases like "cohere webhook", "cohere events",
  "cohere webhook signature", "handle cohere events", "cohere notifications".
...
async function isEventProcessed(eventId: string): Promise<boolean> {
  const key = `cohere:event:${eventId}`;
...
```bash
# Use Cohere CLI to send test events
cohere webhooks trigger resource.created --url http://localhost:3000/webhooks/cohere
...
Configure your webhook URL in the Cohere dashboard.
...
# Send test webhook
curl -X POST https://your-ngrok-url/webhooks/cohere \
  -H "Content-Type: application/json" \
  -d '{"type": "test", "data": {}}'
```

## Resources
- [Cohere Webhooks Guide](https://docs.cohere.com/webhooks)
- [Webhook Security Best Practices](https://docs.cohere.com/webhooks/security)
