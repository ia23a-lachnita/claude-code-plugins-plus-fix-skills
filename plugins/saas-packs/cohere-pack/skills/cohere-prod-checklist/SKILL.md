...
async function healthCheck(): Promise<{ status: string; cohere: any }> {
  ...
  try {
    ...
    return { status: 'healthy', cohere: { connected: true, latencyMs: Date.now() - start } };
  } catch (error) {
    return { status: 'degraded', cohere: { connected: false, latencyMs: Date.now() - start } };
  }
}
```
...
### Immediate Rollback
```bash
kubectl rollout undo deployment/cohere-integration
kubectl rollout status deployment/cohere-integration
```
