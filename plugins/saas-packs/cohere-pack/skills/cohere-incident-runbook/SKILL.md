...
description: |
  ...
  Use when responding to Cohere-related outages, investigating errors,
...
```
Cohere Integration Issue

We're experiencing issues with our Cohere integration.
...
./scripts/cohere-debug-bundle.sh

# Export relevant logs
kubectl logs -l app=cohere-integration --since=1h > incident-logs.txt

# Capture metrics
curl "localhost:9090/api/v1/query_range?query=cohere_errors_total&start=2h" > metrics.json
```

### Postmortem Template
```markdown
## Incident: Cohere [Error Type]
**Date:** YYYY-MM-DD
...
Determine if the issue is Cohere-side or internal.
...
```bash
curl -sf https://api.yourapp.com/health | jq '.services.cohere.status' || echo "UNHEALTHY"
```
